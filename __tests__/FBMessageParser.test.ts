...
describe('FBMessageParser', () => {
    ...
    test('Parser_WhenCalledWithStringNotStartingWithCommandText_ShouldReturnFormatWarning', () => {
        ...
        expect(parser.parse(text)).toEqual(
            {
                command: null,
                error: CommandParserError["Wrong message format, please type 'help' for more infromation..."]
            }
        );
    });

    test('Parser_WhenCalledWithStringNotContainingCommandText_ShouldReturnHelpMessage', () => {
        const text = 'blah blah';
        const parser = new FBMessageParser();
        expect(parser.parse(text)).toEqual(
            {
                command: null,
                error: CommandParserError["Unknown command, please type 'help' for more infromation..."]
            }
        );
    });
